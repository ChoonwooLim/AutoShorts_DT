...
async function showApiKeyModal(modelKey) {
    ...
    console.log(`🔑 API 키 모달 열기: ${modelKey}`);
    ...
    if (!DOM.apiKeyModal || !DOM.apiKeyModalTitle || !DOM.apiKeyInput || !DOM.apiKeyLink) {
        ...
        console.error('DOM 요소들:', {
            apiKeyModal: !!DOM.apiKeyModal,
            apiKeyModalTitle: !!DOM.apiKeyModalTitle,
            apiKeyInput: !!DOM.apiKeyInput,
            apiKeyLink: !!DOM.apiKeyLink
        });
        return;
    }
    ...
    try {
        if (window.apiKeyManager) {
            ...
            console.log(`📦 로드된 키 길이: ${savedApiKey.length}`);
            
            // 메모리에도 반영
            if (aiModels[modelKey]) {
                aiModels[modelKey].apiKey = savedApiKey;
                console.log(`✅ ${modelKey} 모델에 키 반영됨`);
            }
        } else {
            console.warn('⚠️ window.apiKeyManager를 찾을 수 없어 localStorage에서 직접 로드합니다.');
            
            // 환경별 저장소 키 사용
            const isProduction = window.location.hostname === 'twinverse.org' || window.location.hostname === 'www.twinverse.org';
            const storageKey = isProduction ? 'twinverse_apiKeys' : 'apiKeys';
            console.log(`🔍 사용할 저장소 키: ${storageKey}`);
            
            try {
                const storedKeys = JSON.parse(localStorage.getItem(storageKey)) || {};
                savedApiKey = storedKeys[modelKey] || '';
                console.log(`📦 localStorage에서 로드된 키 길이: ${savedApiKey.length}`);
                ...
                console.error('❌ localStorage 읽기 실패:', localStorageError);
                savedApiKey = '';
            }
        }
    } catch (error) {
    ...
    console.log(`✅ API 키 모달 표시 완료: ${modelKey}`);
}
...
async function handleSaveApiKey() {
    ...
    console.log(`🔑 API 키 저장 시작: ${currentApiModelKey}`);
    console.log(`🌍 현재 환경: ${window.location.hostname}:${window.location.port}`);
    ...
    try {
        ...
        if (result.success) {
            ...
            if (aiModels[currentApiModelKey]) {
                ...
                console.log(`✅ ${currentApiModelKey} 모델에 키 반영됨`);
            }
