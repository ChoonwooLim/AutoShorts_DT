...
export async function handleSendMessage() {
    ...
    try {
        ...
        // 사용자가 숏츠 제작을 원하지 않는 경우 체크 (여기서 먼저 정의)
        const noShortsKeywords = ['분석만', '정리만', '요약만', '숏츠 만들지 마', '숏츠 제작하지 마', '숏츠 없이', '분석해줘', '정리해줘', '요약해줘'];
        const wantsShortsKeywords = ['숏츠 만들', '숏츠 제작', '숏츠 추천', '하이라이트 추출'];
        
        const userWantsNoShorts = noShortsKeywords.some(keyword => userInput && userInput.toLowerCase().includes(keyword));
        const userWantsShorts = wantsShortsKeywords.some(keyword => userInput && userInput.toLowerCase().includes(keyword));
...
📋 **중요 지침 - 반드시 따라야 할 규칙:**
1. **절대 숏츠 제작을 제안하지 마세요** - 사용자가 명시적으로 요청하지 않는 한
2. **자막 내용 분석에만 집중하세요** - 요약, 핵심 내용 정리, 주제 파악
3. **숏츠 제안 금지** - "숏츠 1", "하이라이트", "편집 구간" 등의 제안 절대 금지
4. **사용자 요청 우선** - 사용자가 요청한 것만 답변하세요

📊 **현재 프로그램 상황:**
${contextText}

⚠️ **다시 한번 강조: 숏츠 제작 제안을 하지 마세요. 자막 내용 분석만 하세요.**
            `.trim();
...
- 인물 관련 질문 시 병합 정보를 고려한 정확한 답변 제공
        ...
        // 자막 컨텍스트 추가 및 디버깅
        console.log('🔍 자막 컨텍스트 확인:', window.currentSubtitleContext);
        if (window.currentSubtitleContext && window.currentSubtitleContext.data) {
            const subtitleData = window.currentSubtitleContext.data;
            console.log('📝 자막 데이터 발견:', subtitleData);
            systemPrompt += `\n\n📝 **자막 정보:**\n`;
            systemPrompt += `- 파일: ${subtitleData.fileName || '자막 추출됨'}\n`;
            systemPrompt += `- 추출 방법: ${subtitleData.method || '알 수 없음'}\n`;
            systemPrompt += `- 세그먼트 수: ${subtitleData.segments?.length || 0}개\n`;
            if (subtitleData.text) {
                const shortText = subtitleData.text.substring(0, 500);
                const ellipsis = subtitleData.text.length > 500 ? '...' : '';
                systemPrompt += `- 내용 미리보기: "${shortText}${ellipsis}"\n`;
            }
            systemPrompt += `\n📋 **전체 자막 내용:**\n${subtitleData.text || '자막 내용이 없습니다.'}\n`;
            console.log('✅ 시스템 프롬프트에 자막 추가됨');
        } else {
            console.log('❌ 자막 컨텍스트가 없습니다');
        }
        
        // 이전 대화 내용 추가 (최근 3개만)
        const previousMessages = currentChat.messages.slice(-6, -1); // 최근 3쌍의 대화
        if (previousMessages.length > 0) {
            const recentChats = previousMessages.map(msg => {
                const role = msg.role === 'user' ? '👤 사용자' : '🤖 AI';
                const content = msg.content.substring(0, 100);
                const ellipsis = msg.content.length > 100 ? '...' : '';
                return `${role}: ${content}${ellipsis}`;
            }).join('\n');
            systemPrompt += `\n\n📝 **최근 대화:**\n${recentChats}\n`;
        }
        ...
        if (videoFrames.length > 0) {
            ...
            // 사용자가 명시적으로 숏츠를 원하지 않는 경우
            if (userWantsNoShorts || isSubtitleAnalysisRequest) {
                aiMessage = `**동영상/자막 분석 요청**\n\n질문: "${userInput || '동영상 내용 분석'}"\n\n**📋 요청사항:**\n- 사용자가 요청한 내용만 분석해주세요\n- 숏츠 제작 제안은 하지 마세요\n- 자막이나 영상 내용 분석에 집중해주세요`;
            } 
            // 사용자가 명시적으로 숏츠를 원하는 경우 또는 명확하지 않은 경우
            else if (userWantsShorts || !userInput) {
                // 🚨 상세한 설명을 위한 개선된 프롬프트
        ...
        // AI 응답에서 숏츠 제안 자동 감지 및 처리 (사용자가 원할 때만)
        if (!userWantsNoShorts && !isSubtitleAnalysisRequest) {
            try {
            ...
            console.log('📝 사용자가 자막 분석만 요청했으므로 숏츠 제안 처리를 건너뜁니다.');
        }
    } catch (error) {
