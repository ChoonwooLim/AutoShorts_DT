...
class ApiKeyManager {
    ...
    detectEnvironment() {
        ...
        if (hostname === 'twinverse.org' || hostname === 'www.twinverse.org') {
            ...
            return 'development-vite';
        } else if (hostname === 'localhost' && port === '3000') {
            return 'development-express';
        } else {
            return 'unknown';
        }
    }
    ...
    async saveApiKey(provider, apiKey) {
        ...
        console.log(`💾 API 키 저장 시도: ${provider}`);
        console.log(`🌍 현재 환경: ${this.environment}`);
...
async function callClaudeAPI(message, systemPrompt, modelData, subModel, imagesData = []) {
    ...
    const response = await fetch(modelData.endpoint, {
        ...
        body: JSON.stringify({
            ...
            // 🚨 TPM 최적화: max_tokens 줄임 (4096 → 2000)
            max_tokens: imagesData && imagesData.length > 0 ? 1000 : 2000,
            system: systemPrompt,
...
async function callGenericOpenAIAPI(message, systemPrompt, modelData, subModel, modelMap, imagesData = []) {
    ...
    const response = await fetch(modelData.endpoint, {
        ...
        body: JSON.stringify({
            ...
            // 🚨 TPM 제한 해결: max_tokens 명시적 설정
            max_tokens: imagesData && imagesData.length > 0 ? 1000 : 2000, // 이미지 있을 때는 더 적게
        })
    });
    if (!response.ok) {
        const err = await response.json().catch(() => ({error:{message: response.statusText}}));
        
        // 🚨 TPM 제한 오류 감지 및 자세한 안내
        if (response.status === 429) {
            const errorMessage = err.error?.message || '';
            if (errorMessage.includes('tokens') || errorMessage.includes('TPM')) {
                throw new Error(`🚫 TPM 제한 초과 오류\n\n📊 60프레임 분석으로 요청량이 증가했습니다. 다음 해결책을 시도해보세요:\n\n✅ 즉시 해결:\n• **Gemini 모델로 변경** (TPM 제한 없음, 권장)\n• 1-2분 후 다시 시도\n• checkVideoAnalysisSettings() 로 현재 설정 확인\n\n🔧 근본 해결:\n• OpenAI 요금제 업그레이드 (Tier 1 이상 권장)\n• https://platform.openai.com/usage-limits\n\n📊 개선된 분석:\n• 모든 모델: 60프레임으로 향상 (기존 25-30개에서)\n• 더 정밀한 동영상 분석 가능\n• Gemini 모델 사용 시 제한 없음\n\n원본 오류: ${errorMessage}`);
            }
        }
