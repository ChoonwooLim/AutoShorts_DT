...
async function handleFileSelect(file) {
    ...
    try {
        ...
        const hasSelectedPlatform = Array.from(platformCards).some(card => card.classList.contains('selected'));
        
        if (!hasSelectedPlatform && platformCards.length > 0) {
            platformCards[0].classList.add('selected');
            console.log('✅ 첫 번째 플랫폼 카드가 자동 선택되었습니다.');
        }
